...
> iteration get written back here (Phase 7).

Contribution claim (one sentence):
Target venue (author's call): 
Output language:
...
- Flags: `⚠needs-method` / `⚠needs-analysis` / `❓citation-unverified`
- *(core causal/eliminative nodes only)* **Load-bearing assumptions / open rebuttals:**

<repeat nodes per section>

---

## Load-bearing assumptions & limitations (summary)
> Filled from Phase 4.5 if run, or from the moves' open-rebuttals. Feeds the
