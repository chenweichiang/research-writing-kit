...
- **With git:** `git clone <the kit's GitHub URL>` — the person who invited them has the
  URL. (If `git` is missing, macOS will offer to install the developer tools; accept it,
  or just use the ZIP below.)
- **Without git:** on the kit's GitHub page, green **Code** button → **Download ZIP** →
  unzip anywhere (e.g. Downloads).
